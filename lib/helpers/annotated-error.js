...
function errMark(params) {
  ...
  if (typeof params.linenum === 'number' && !isNaN(params.linenum)) {
    var lines = params.source.split('\n'),
    ...
    debug.push(params.message);
  } else {
    debug.push(params.source, params.message);
  }
...
module.exports = function(file, error, source, linenum, column) {
  if (typeof file === 'object') {
    // TODO: more reporters for all engines?
    if (error.reason && error.source) {
      linenum = error.line;
      column = error.column;
    }
    ...
    error = error.message;
    source = error.source || file.source;
    file = error.file || file.filename;
  }
