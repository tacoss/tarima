...
function bundle(partial, locals, bundleOpts, cb) {
  partial.render(locals, function(err, result) {
    var isTemplate = partial.params.parts.filter(support.isTemplate).length,
        isScript = partial.params.ext === 'js' && support.isCJS(result.code);

    result.code = ensureWrapper(result.code);
