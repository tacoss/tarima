...
function bundle(partial, locals, bundleOpts, cb) {
  ...
  // dirty check
  partial.params._import = true;
  partial.params._nopost = !partial.params.isTemplate;
      ...
      tasks.push(function(next) {
        view.render(locals, function(err, result) {
          if (!err) {
            ...
            if (!(support.isJSON(result.source) || support.isTemplateFunction(result.source))) {
              result.source = tosource(result.source.trim());
            }
