...
var re_vars = /\s*,\s*/g;
var re_clean_vars = /\{\s*\}/;
var re_scope = /___scope___\.file\("(.+?)"/g;
...
module.exports = function(options, params, done) {
  ...
  fsbx.FuseBox.init({
    ...
    plugins: [{
      ...
      transform: function(file) {
        ...
        const x = parse(file.info.absPath, file.contents.toString(), params.options);

        return render(x, (err, result) => {
  ...
  .then(function(result) {
    ...
    if (params.source.match(re_scope)) {
      params.source = params.source.replace(re_scope, function(_, $1) {
        if ($1 !== 'default.js') {
          // FIXME
