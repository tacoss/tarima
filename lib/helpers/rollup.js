...
module.exports = function(options, params, done) {
  ...
  Rollup.rollup({
    ...
    // TODO: consider moving this to a proper place
    if (Array.isArray(params.data._prelude)) {
      params.source = [
        '(function() {',
        params.data._prelude.map(function(prelude) {
          return options.prelude[prelude] || '';
        }).join(';'),
        params.source,
        '}());'
      ].join('\n');
