...
function _loader(source, map) {
  const cb = this.async();
  const params = parse(this.resourcePath, source, _loader.params);

  render(params, (err, result) => cb(err, result.source, result.sourceMap || map));
}
