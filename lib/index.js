...
function partial(params) {
  ...
  function render(locals, raw) {
    ...
    if (view.filepath && locals[RENDER_KEY]) {
      ...
      tpl = tarima.load(path.resolve(view.filepath, tpl));

      var sub = tpl.data($.merge({}, locals, { yield: view.source }));

      if (sub.required) {
        view.required = view.required || [];
        sub.required.forEach(function(dep) {
...
tarima.parse = function(src, code, options) {
  ...
  if (matches) {
    ...
    options.data = yaml.load(data, { filename: src });
