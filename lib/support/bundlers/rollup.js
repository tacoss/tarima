...
module.exports = (options, params, done) => {
  ...
  Rollup.rollup({
    ...
    external: toArray(params.data._external).concat(toArray(rollupOptions.external))
      .concat(toArray(Object.keys(options.external))),
    ...
    const output = bundle.generate({
      ...
      globals: merge({}, params.data._globals, rollupOptions.globals, options.external),
    });
