  ...
  if (!params.next || params.next === 'js') {
    ...
    if (params.source.indexOf('<template') > -1 && params.source.indexOf('</template>') > -1) {
      ...
      .then(() => {
        ...
        // FIXME: Moon.component is not working?
        params.source += ` function (p) {
          var o = {}; Object.keys(__m$).forEach(function(k) { o[k] = __m$[k] || o[k]; });
          p && Object.keys(p).forEach(function(k) { o[k] = p[k] || o[k]; });
          return new Moon(o);
        };`;
