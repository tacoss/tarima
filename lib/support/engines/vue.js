...
function render(tasks, self, params, view, out, key) {
  if (!view[key]) {
    out[key] = '';
    return;
  }
...
function compile(params, cb) {
  ...
  if (params.source.indexOf('<template') === -1 && params.source.indexOf('</template>') === -1) {
    ...
    render(tasks, this, params, view, _target, 'template');
    render(tasks, this, params, view, _target, 'script');

    if (view.styles && view.styles.length) {
      view.styles.forEach(function(style, key) {
        render(tasks, this, params, view.styles, _target.styles, key);
      }, this);
    }

    map(tasks, function(err) {
      if (!err) {
        try {
