...
var reTemplateFunction = /^\s*(?:function.*?\(|Handlebars\.template)/,
    reJSONValidate = /^(?:\{[\s\S]*\}|"[\s\S]*"|\[[\s\S]*\]|-?\d.*|true|false|null)$/;
...
var isTemplate = reTemplateFunction.test.bind(reTemplateFunction),
    isJSON = reTemplateFunction.test.bind(reJSONValidate);

module.exports.isTemplateFunction = function(source) {
  return isJSON(source) || isTemplate(source);
};
