...
function getDynamicPlugin(rollupOptions, options, params) {
  ...
  return {
    ...
    resolveId(importee, importer) {
      ...
      if (isDir(fixedModule) && isFile(path.join(fixedModule, 'index.js'))) {
        fixedModule = path.join(fixedModule, 'index.js');
      }

      if (isFile(fixedModule)) {
        return fixedModule;
      }
      ...
      for (let j = 0; j < folders.length; j += 1) {
        fixedModule = path.join(folders[j], importee);
