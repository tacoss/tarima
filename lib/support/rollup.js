...
module.exports = (options, params, done) => {
  ...
  Rollup.rollup({
    ...
    experimentalPreserveModules: rollupOptions.experimentalPreserveModules,
    experimentalTopLevelAwait: rollupOptions.experimentalTopLevelAwait,
    ...
    return bundle.generate({
      ...
      .then(result => {
        ...
        Object.keys(result.output).forEach(k => {
          Object.keys(result.output[k].modules).forEach(dep => {
            if (params.filename !== dep) {
              params.deps.push(dep);
            }
          });

          _code[k] = result.output[k].code;
          _maps[k] = result.output[k].map;
        });

        params._bundle = bundle;
        params.source = _code;
        params.sourceMap = _maps;
