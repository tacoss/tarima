...
function getDynamicPlugin(rollupOptions, options, params) {
  ...
  return {
    ...
    resolveId(importee, importer) {
      ...
      if (builtinAliases.indexOf(importee) !== -1) {
        return require.resolve(importee + '.mjs');
      }
...
function getDefaultsFrom(rollupOptions, options, params) {
  ...
  return {
    ...
    plugins: loadPlugins(rollupOptions.plugins || [], rollupOptions).concat([getDynamicPlugin(rollupOptions, options, params)]),
  };
}
