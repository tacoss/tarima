...
function errMark(params) {
  ...
  if (params.message.indexOf(params.filename) === -1) {
    ...
    params.message = params.message.replace(/^(.+?:\d+)/, '$1 (' + params.exception + ')');
  }
  ...
  if (params.errorstack) {
    debug.push(params.errorstack.split('\n').filter(function(line) {
      return /^\s+at\b/.test(line);
    }).join('\n'));
  }
...
module.exports = function(err, params) {
  var fixedError = new Error(),
      fixedType = err.type || err.name,
      fixedFile = path.join(params.filepath, params.filename);

  fixedError.stack = params.options.verboseDebug ? err.stack || fixedError.stack : null;

  if (err.mark) {
    fixedError.message = errMark({
      errorstack: fixedError.stack,
      exception: fixedType,
      filename: fixedFile,
      message: err.reason,
      extract: err.mark.buffer,
      linenum: err.mark.line
    });
  } else if (err.lexer) {
    fixedError.message = errMark({
      errorstack: fixedError.stack,
